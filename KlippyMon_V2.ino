...
void handleTimeUsed() {
  ...
  tft.drawString(buffer, 120, 260, 4);  // and draw the time the print actually took
}
//
void handleETA() {
  ...
  if (printState == "printing") {           // if we are printing something, give me a time
    if ((progress * 100) > lastProgress) {  // the percentage done has increased
      ...
      tft.setTextColor(TFT_WHITE, TFT_BLACK);                   // white text
      tft.fillRect(1, 230, 239, 32, TFT_BLACK);                 // remove the old ETA first
      tft.drawString("ETA: " + String(timeLeft), 120, 260, 4);  // ETA when print will be done
...
void estimateTimeRemaining(float elapsedSeconds, float percentComplete, char *result) {
  ...
  unsigned int hh = remaining / 3600;
  unsigned int mm = (remaining % 3600) / 60;
  //unsigned int ss = remaining % 60;

  snprintf(result, 16, "%02u:%02u", hh, mm);
}
//
      ...
      if (error) {                                                 // if there was a data error, well, JSON error!
        ...
        if (printState == "standby") {                 // if the printer is in standy, it's powered up but idle, not printing
          ...
          if (gotStartTime == true) {                  // we just finished a print job
            ...
            tft.fillRect(1, 270, 239, 26, TFT_BLACK);  // get rid of the filename we just printed
          }
          ...
          if (thePrintFile == "") {                                                        // if the name is blank, go get it. once.
            ...
            tft.setTextDatum(BC_DATUM);
            tft.drawString(thePrintFile, 120, 294, 2);  // show the name of the file printing
          ...
          if (gotStartTime == false) {                 // grab the start time
            ...
            tft.fillRect(1, 230, 239, 32, TFT_BLACK);  // remove the old total (actual print time) of print time first
          }
          ...
          } else {                                                                            // at this point, we are either heating the nozzle or printing
